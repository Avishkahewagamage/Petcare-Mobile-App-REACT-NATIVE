...
const TrainerScheduleScreen = ({ route }) => {
    ...
    useEffect(() => {
        const fetchTrainerSchedule = async () => {
            try {
                const response = await fetch(`http://192.168.1.54:8084/api/schedules/trainer/${trainerId}`);
                ...
                {loading ? (
                    ...
                    <View style={styles.scheduleContainer}>
                        <Text style={styles.title}>
                            <Icon name="calendar" size={24} color="#4CAF50" /> Schedule {schedule.trainer.username}
                        </Text>
